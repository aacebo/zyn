...
use zyn::quote::{format_ident, quote};
...
fn parse_vanilla(b: &mut Bencher) {
    let ts = make_input();
    b.iter(|| zyn::syn::parse2::<zyn::syn::DeriveInput>(black_box(ts.clone())).unwrap())
}

#[bench]
fn parse_zyn(b: &mut Bencher) {
    let ts = make_input();
    b.iter(|| zyn::syn::parse2::<Input>(black_box(ts.clone())).unwrap())
}

#[bench]
fn extract_vanilla(b: &mut Bencher) {
    let ts = make_input();
    let derive: zyn::syn::DeriveInput = zyn::syn::parse2(ts).unwrap();
    b.iter(|| {
        let zyn::syn::Data::Struct(ref s) = black_box(&derive).data else {
            panic!()
        };
        let zyn::syn::Fields::Named(ref n) = s.fields else {
            panic!()
        };
        black_box(n.clone())
    })
...
fn extract_zyn(b: &mut Bencher) {
    let ts = make_input();
    let input: Input = zyn::syn::parse2(ts).unwrap();
    b.iter(|| black_box(Fields::<zyn::syn::FieldsNamed>::from_input(black_box(&input)).unwrap()))
}
...
fn codegen_vanilla(b: &mut Bencher) {
    let ts = make_input();
    let derive: zyn::syn::DeriveInput = zyn::syn::parse2(ts).unwrap();
    let zyn::syn::Data::Struct(ref data) = derive.data else {
        panic!()
    };
    let zyn::syn::Fields::Named(ref named) = data.fields else {
        panic!()
    };
...
fn codegen_zyn(b: &mut Bencher) {
    let ts = make_input();
    let input: Input = zyn::syn::parse2(ts).unwrap();
    let fields = Fields::<zyn::syn::FieldsNamed>::from_input(&input).unwrap();
    b.iter(|| {
        black_box(zyn::zyn! {
            impl UserRecord {
...
fn full_vanilla(b: &mut Bencher) {
    let ts = make_input();
    b.iter(|| {
        let derive: zyn::syn::DeriveInput = zyn::syn::parse2(ts.clone()).unwrap();
...
fn full_zyn(b: &mut Bencher) {
    let ts = make_input();
    b.iter(|| {
        let input: Input = zyn::syn::parse2(ts.clone()).unwrap();
...
fn enum_extract_vanilla(b: &mut Bencher) {
    ...
    let derive: zyn::syn::DeriveInput = zyn::syn::parse2(ts).unwrap();
    b.iter(|| {
        let zyn::syn::Data::Enum(ref e) = black_box(&derive).data else {
            panic!()
        };
...
fn enum_codegen_vanilla(b: &mut Bencher) {
    let ts = make_enum_input();
    let derive: zyn::syn::DeriveInput = zyn::syn::parse2(ts).unwrap();
    let zyn::syn::Data::Enum(ref data) = derive.data else {
        panic!()
    };
